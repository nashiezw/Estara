...
function AgencyRow({ agency, plans, busy, send }: any) {
  ...
  return <article className={`platform-agency-row platform-agency-card${needsCare(agency) ? " needs-care" : ""}`}>
    ...
    {open && <div className="platform-manage-panel">{actions.map(([label, disabled, fn]) => <button key={label} disabled={busy || disabled} onClick={fn}>{label}</button>)}{confirm && <span className="platform-confirm"><small>Confirm {confirm.label}</small><button disabled={busy} onClick={() => { confirm.fn(); setConfirm(null); }}>Confirm</button><button onClick={() => setConfirm(null)}>Cancel</button></span>}</div>}
  </article>;
}
