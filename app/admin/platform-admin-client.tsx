...
export default function PlatformAdminClient({ displayName }: { displayName: string }) {
  ...
  return <main className={`platform-admin platform-admin-premium platform-admin-compact${menuOpen ? " platform-menu-open" : ""}`} style={adminTheme}>
    ...
    <section className="platform-main">
      ...
      {tab === "billing" && <Billing agencies={data.agencies} plans={data.plans} invoices={data.invoices} coupons={data.coupons} paymentMethods={data.paymentMethods} manualPayments={data.manualPayments} busy={busy} send={send} />}
...
function Billing({ agencies, plans = [], invoices, coupons, paymentMethods = [], manualPayments = [], busy, send }: any) {
  ...
  const moneyMinor = (rows: any[]) => rows.reduce((sum: number, item: any) => sum + Number(item.totalMinor ?? item.amountDueMinor ?? 0), 0), fmt = (minor: number) => `USD ${(minor / 100).toFixed(2)}`;
  const planChoices = plans.filter((plan: any) => plan.status === "published"), allowedPlanIds = method.allowedPlanVersionIds.split(",").map(x => x.trim()).filter(Boolean);
  ...
  const toggleAllowedPlan = (id: string, checked: boolean) => setMethod(current => ({ ...current, allowedPlanVersionIds: (checked ? [...new Set([...allowedPlanIds, id])] : allowedPlanIds.filter(planId => planId !== id)).join(",") }));
  ...
  return <div className="platform-revenue-grid revenue-desk">
    <section className="platform-card revenue-hero"><div><span>REVENUE DESK</span><h2>Billing control, proof review and collection readiness.</h2><p>Finance sees what is due, what is waiting for evidence, and which channels agencies can use before a subscription is activated.</p></div><aside><strong>{fmt(moneyMinor(open))}</strong><small>Open invoice value</small></aside></section>
    <section className="revenue-strip"><article><span>Proof queue</span><strong>{queue.length}</strong><small>Needs review</small></article><article><span>Paid ledger</span><strong>{fmt(moneyMinor(paid))}</strong><small>{paid.length} paid invoices</small></article><article><span>Channels</span><strong>{paymentMethods.filter((x: any) => x.enabled).length}</strong><small>{paymentMethods.length} configured</small></article><article><span>Open invoices</span><strong>{open.length}</strong><small>Awaiting settlement</small></article></section>
    <section className="platform-card platform-payment-queue revenue-panel"><span>MANUAL PAYMENT REVIEW</span><h2>Proof queue</h2><p className="revenue-panel-copy">Approve only after the proof, reference and amount match the subscription invoice.</p><div className="platform-mini-kpis"><span><strong>{queue.length}</strong><small>Awaiting finance review</small></span><span><strong>{manualPayments.filter((x: any) => x.status === "approved").length}</strong><small>Approved requests</small></span></div>{manualPayments.length ? manualPayments.map((item: any) => <article className={`payment-review payment-${item.status}`} key={item.id}><header><span><strong>{item.agency}</strong><small>{item.planName} · {item.paymentMethod || item.provider}</small></span><b>{item.currency} {(item.amountDueMinor / 100).toFixed(2)}</b><em>{item.status.replaceAll("_", " ")}</em></header><p>Reference {item.paymentReference}{item.transactionReference ? ` · transaction ${item.transactionReference}` : ""}{item.proofOriginalName ? ` · proof ${item.proofOriginalName}` : ""}</p>{["proof_submitted", "under_review"].includes(item.status) && <div className="payment-review-actions"><input placeholder="Finance note" value={reviewNote[item.id] || ""} onChange={event => setReviewNote({ ...reviewNote, [item.id]: event.target.value })} />{item.proofObjectKey && <a href={`/api/platform/billing-proof?id=${encodeURIComponent(item.id)}`} target="_blank" rel="noreferrer">Open proof</a>}<button disabled={busy} onClick={() => review(item.id, "approve")}>Approve</button><button disabled={busy} onClick={() => review(item.id, "resubmit")}>Request resubmit</button><button disabled={busy} className="danger" onClick={() => review(item.id, "reject")}>Reject</button></div>}</article>) : <EmptyPanel title="No manual proof submitted" detail="Payment requests appear here after an agency uploads proof." />}</section>
    <section className="platform-card platform-form platform-payment-methods"><span>PAYMENT METHODS</span><h2>Collection channels</h2><p>Configure the payment details agencies must see at checkout. Saved methods are managed in the catalogue below the editor.</p><div><label>Name<input value={method.name} onChange={event => setMethod({ ...method, name: event.target.value })} /></label><label>Type<select value={method.type} onChange={event => setMethod({ ...method, type: event.target.value })}><option value="bank_transfer">Bank transfer</option><option value="ecocash">EcoCash</option><option value="onemoney">OneMoney</option><option value="innbucks">InnBucks</option><option value="cash">Cash</option><option value="mobile_money">Mobile money</option><option value="stripe">Stripe</option><option value="other">Other</option></select></label><label>Currency<input maxLength={3} value={method.currency} onChange={event => setMethod({ ...method, currency: event.target.value.toUpperCase() })} /></label><label>Display order<input type="number" value={method.displayOrder} onChange={event => setMethod({ ...method, displayOrder: Number(event.target.value) })} /></label><label>Account holder<input value={method.accountHolder} onChange={event => setMethod({ ...method, accountHolder: event.target.value })} /></label><label>Bank / provider<input value={method.bankName} onChange={event => setMethod({ ...method, bankName: event.target.value })} /></label><label>Account / merchant<input value={method.accountNumber} onChange={event => setMethod({ ...method, accountNumber: event.target.value })} /></label><label>Mobile number<input value={method.mobileNumber} onChange={event => setMethod({ ...method, mobileNumber: event.target.value })} /></label><label className="wide">Payment instructions<input value={method.instructions} onChange={event => setMethod({ ...method, instructions: event.target.value })} /></label><label className="wide">Reference instructions<input value={method.referenceInstructions} onChange={event => setMethod({ ...method, referenceInstructions: event.target.value })} /></label><label>Currencies<input value={method.currencies} onChange={event => setMethod({ ...method, currencies: event.target.value })} /></label><label className="platform-plan-toggle"><input type="checkbox" checked={method.enabled} onChange={event => setMethod({ ...method, enabled: event.target.checked })} /><span>Enabled for agencies</span></label><fieldset className="payment-plan-picker"><legend>Allowed plans</legend><label className="platform-plan-toggle"><input type="checkbox" checked={!allowedPlanIds.length} onChange={() => setMethod({ ...method, allowedPlanVersionIds: "" })} /><span>All published plans</span></label>{planChoices.map((plan: any) => <label className="platform-plan-toggle" key={plan.id}><input type="checkbox" checked={allowedPlanIds.includes(plan.id)} onChange={event => toggleAllowedPlan(plan.id, event.target.checked)} /><span>{plan.name} v{plan.version}</span></label>)}</fieldset></div><button disabled={busy || !method.name} onClick={saveMethod}>{busy ? "Saving..." : method.id ? "Save payment method" : "Add payment method"}</button>{method.id && <button type="button" className="payment-method-reset" onClick={() => setMethod({ ...method, id: "", name: "", accountHolder: "", bankName: "", accountNumber: "", merchantNumber: "", mobileNumber: "", instructions: "", referenceInstructions: "" })}>Create new method</button>}<div className="payment-method-catalogue"><span>SAVED CHANNELS</span><div className="payment-method-list">{paymentMethods.map((item: any) => <button type="button" key={item.id} onClick={() => editMethod(item)}><strong>{item.name}</strong><small>{item.type.replaceAll("_", " ")} · {item.currency} · {item.enabled ? "enabled" : "disabled"}</small>{(item.mobileNumber || item.accountNumber || item.merchantNumber) && <em>{item.mobileNumber || item.accountNumber || item.merchantNumber}</em>}</button>)}</div></div></section>
    <section className="platform-card platform-ledger-card revenue-panel"><span>INVOICES & RECEIPTS</span><h2>Billing ledger</h2><p className="revenue-panel-copy">Issue invoices from assigned subscription plans and record real provider references only after settlement evidence exists.</p><div className="platform-mini-kpis"><span><strong>{open.length}</strong><small>Open invoices</small></span><span><strong>{paid.length}</strong><small>Paid invoices</small></span></div><div className="invoice-create"><select value={agencyId} onChange={event => setAgencyId(event.target.value)}>{agencies.filter((x: any) => x.subscriptionId).map((item: any) => <option value={item.id} key={item.id}>{item.name}</option>)}</select><input placeholder="Coupon (optional)" value={couponCode} onChange={event => setCouponCode(event.target.value)} /><button disabled={busy || !agencyId} onClick={() => send("POST", { action: "create_invoice", agencyId, couponCode })}>{busy ? "Issuing..." : "Issue invoice"}</button></div>{invoices.map((invoice: any) => <article className="invoice-row" key={invoice.id}><span><strong>{invoice.invoiceNumber}</strong><small>{invoice.agency} · due {new Date(invoice.dueAt).toLocaleDateString()}</small></span><b>{invoice.currency} {(invoice.totalMinor / 100).toFixed(2)}</b><em>{invoice.status}</em>{invoice.status === "open" && <button disabled={busy} onClick={() => setReceipt({ invoiceId: invoice.id, reference: receipt.invoiceId === invoice.id ? receipt.reference : "" })}>{busy ? "Opening..." : "Record payment"}</button>}{receipt.invoiceId === invoice.id && <div className="invoice-receipt"><label>Receipt or provider reference<input autoFocus value={receipt.reference} onChange={event => setReceipt({ ...receipt, reference: event.target.value })} /></label><button disabled={busy || !receipt.reference.trim()} onClick={recordPayment}>{busy ? "Saving..." : "Save receipt"}</button><button className="invoice-secondary" disabled={busy} onClick={() => setReceipt({ invoiceId: "", reference: "" })}>Cancel</button></div>}</article>)}</section>
    <section className="platform-card platform-form revenue-coupons"><span>DISCOUNTS</span><h2>Coupons</h2><p className="revenue-panel-copy">Use coupons as controlled commercial exceptions. Every redemption remains visible in the billing ledger.</p><label>Code<input value={coupon.code} onChange={event => setCoupon({ ...coupon, code: event.target.value })} /></label><label>Type<select value={coupon.kind} onChange={event => setCoupon({ ...coupon, kind: event.target.value })}><option value="percent">Percentage</option><option value="fixed">Fixed cents</option></select></label><label>Amount<input type="number" min="1" value={coupon.amount} onChange={event => setCoupon({ ...coupon, amount: Number(event.target.value) })} /></label><button disabled={busy || !coupon.code} onClick={() => send("POST", { action: "create_coupon", ...coupon })}>{busy ? "Creating coupon..." : "Create coupon"}</button><div className="coupon-list">{coupons.map((item: any) => <span key={item.code}><strong>{item.code}</strong><small>{item.kind === "percent" ? `${item.amount}%` : `${item.amount} cents`} · {item.redemptions} used</small></span>)}</div></section>
  </div>;
}
