...
function Plans({ rows, busy, send }: any) {
  ...
  const loadPlan = (plan: any) => setForm({ id: plan.id, planKey: plan.planKey, name: plan.name, currency: plan.currency, priceMinor: plan.priceMinor, billingPeriod: plan.billingPeriod, status: plan.status, entitlements: { ...defaultPlanForm.entitlements, ...(plan.entitlements || {}) }, limits: { ...defaultPlanForm.limits, ...(plan.limits || {}) } });
  ...
  const payload = () => ({ id: form.id, planKey: form.planKey, name: form.name, currency: form.currency, priceMinor: Number(form.priceMinor), billingPeriod: form.billingPeriod, status: form.status, entitlements: form.entitlements, limits: form.limits });
  ...
  return <div className="platform-plan-workbench">
    ...
    <form className="platform-card platform-plan-editor" onSubmit={submit}><span>{form.id ? form.status === "draft" ? "EDIT DRAFT VERSION" : "LOCKED VERSION PREVIEW" : "CREATE A NEW VERSION"}</span><h2>Plan definition</h2>{form.id && form.status !== "draft" && <p className="platform-plan-warning">Published and archived versions are locked. Use Create new version before changing a live contract.</p>}<div className="platform-plan-fields"><label>Plan key<input disabled={Boolean(form.id)} value={form.planKey} onChange={event => setForm({ ...form, planKey: event.target.value })} /></label><label>Name<input disabled={form.id && form.status !== "draft"} value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} /></label><label>Currency<input disabled={form.id && form.status !== "draft"} maxLength={3} value={form.currency} onChange={event => setForm({ ...form, currency: event.target.value })} /></label><label>Price, cents<input disabled={form.id && form.status !== "draft"} type="number" min="0" value={form.priceMinor} onChange={event => setForm({ ...form, priceMinor: Number(event.target.value) })} /></label><label>Billing period<select disabled={form.id && form.status !== "draft"} value={form.billingPeriod} onChange={event => setForm({ ...form, billingPeriod: event.target.value })}><option value="month">Monthly</option><option value="year">Yearly</option></select></label><label>Status<select disabled={Boolean(form.id)} value={form.status} onChange={event => setForm({ ...form, status: event.target.value })}><option value="draft">Draft</option><option value="published">Published</option></select></label></div><fieldset><legend>Feature access</legend>{Object.entries(planFeatureLabels).map(([key, label]) => <label className="platform-plan-toggle" key={key}><input disabled={form.id && form.status !== "draft"} type="checkbox" checked={Boolean(form.entitlements[key])} onChange={event => updateEntitlement(key, event.target.checked)} /><span>{label}</span></label>)}</fieldset><fieldset><legend>Usage limits</legend>{Object.entries(planLimitLabels).map(([key, label]) => <label key={key}>{label}<input disabled={form.id && form.status !== "draft"} type="number" min="0" value={form.limits[key] ?? 0} onChange={event => updateLimit(key, event.target.value)} /></label>)}</fieldset><button disabled={busy || (form.id && form.status !== "draft")}>{busy ? "Saving plan..." : form.id ? "Save draft version" : "Create plan version"}</button></form>
