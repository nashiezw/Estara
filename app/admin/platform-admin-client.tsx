...
function AgencyRow({ agency, plans, busy, send }: any) {
  ...
  return <article className={`platform-agency-row platform-agency-card${needsCare(agency) ? " needs-care" : ""}`}><header><div className="platform-agency-mark">{agency.name.split(/\s+/).map((x: string) => x[0]).join("").slice(0, 2).toUpperCase()}</div><span><strong>{agency.name}</strong><small>{agency.slug} · created {dateShort(agency.createdAt)}</small></span><span className="platform-badge-stack"><em className={`state-${agency.state || "unassigned"}`}>{subLabel(agency.state)}</em><em className={`state-${agency.agencyStatus || "active"}`}>{tenantLabel(agency.agencyStatus || "active")}</em></span></header><div className="platform-agency-metrics"><span><b>{agency.properties}</b><small>properties</small></span><span><b>{agency.users}</b><small>users</small></span><span><b>{agency.enquiries}</b><small>enquiries</small></span><span><b>{agency.viewings || 0}</b><small>viewings</small></span></div><div className="platform-agency-plan"><label>Plan<select aria-label={`Plan for ${agency.name}`} value={plan} onChange={event => setPlan(event.target.value)}>{plans.map((item: any) => <option value={item.id} key={item.id}>{item.name} v{item.version}</option>)}</select></label><button disabled={busy || !plan} onClick={() => send("POST", { action: "assign_subscription", agencyId: agency.id, planVersionId: plan, state: agency.state === "active" ? "active" : "trialing" })}>{busy ? "Applying..." : agency.subscriptionId ? "Apply plan" : "Start trial"}</button></div><footer><small>{statusInfo || "No lifecycle note"}</small><button onClick={() => setOpen(value => !value)}>{open ? "Close management" : "Manage tenant"}</button></footer>{open && <div className="platform-manage-panel"><a href={`/site/${agency.slug}`} target="_blank">View site</a>{actions.map(([label, disabled, fn]) => <button key={label} disabled={busy || disabled} onClick={fn}>{label}</button>)}{confirm && <span className="platform-confirm"><small>Confirm {confirm.label}</small><button disabled={busy} onClick={() => { confirm.fn(); setConfirm(null); }}>Confirm</button><button onClick={() => setConfirm(null)}>Cancel</button></span>}</div>}</article>;
}
