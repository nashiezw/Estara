...
async function GET() { const started = Date.now(); try {
    ...
    const backupRun = await runDueBackups(), leaseLifecycle = await runLeaseLifecycleAutomation();
