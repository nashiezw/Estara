...
import { productionProviderReadiness } from "../../../db/production-providers";
...
async function GET() { const started = Date.now(); try {
    ...
    const platform = await getPlatformIdentity(), providerReadiness = productionProviderReadiness(env as Record<string, string | undefined>), backupRun = await runDueBackups(), leaseLifecycle = await runLeaseLifecycleAutomation();
    ...
    return Response.json({ status, checkedAt: new Date().toISOString(), platform: { shortName: platform.shortName }, latencyMs: { total: Date.now() - started, database: databaseMs, storage: storageMs }, services: { database: database?.healthy === 1 ? "operational" : "failed", storage: probe ? "operational" : "failed", backups: backupRun.some(x => x.status === "failed") ? "degraded" : "operational" }, providers: providerReadiness, queues: { events: events.results, automations: automations.results, deliveries: deliveries.results, renders: renders.results, deadLetters }, backups: { processed: backupRun.length, results: backupRun }, leaseLifecycle, usage: counts }, { status: status === "unhealthy" ? 503 : 200, headers: { "cache-control": "private, no-store" } });
}
catch (error) {
