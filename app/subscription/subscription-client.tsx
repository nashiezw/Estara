...
export default function SubscriptionClient({ platform }: { platform: PlatformBrand }) {
  ...
  return <>
    ...
    <section className="subscription-shop">
      ...
      <div className="subscription-plan-options">{data.availablePlans.map((item: any) => <button className={selectedPlan === item.id ? "selected" : ""} key={item.id} onClick={() => { setSelectedPlan(item.id); setSelectedMethod(""); }}><span>{item.featured ? "Recommended" : "Plan"}</span><strong>{item.name}</strong><b>{money(item.currency, item.priceMinor)}<small>/{item.billingPeriod}</small></b><small>{item.description || `${item.trialAvailable ? `${item.trialDays} day trial available` : "No trial"} · version ${item.version}`}</small></button>)}</div>
      {selected && <div className="subscription-checkout"><section><span>NEXT STEP</span><h3>{selected.name}</h3>{selected.priceMinor === 0 ? <button disabled={busy === "start_free"} onClick={() => requestJson("start_free", { planVersionId: selected.id })}>{busy === "start_free" ? "Starting free plan..." : "Start free plan"}</button> : <><button disabled={!selected.trialAvailable || busy === "start_trial"} onClick={() => requestJson("start_trial", { planVersionId: selected.id })}>{selected.trialAvailable ? busy === "start_trial" ? "Starting trial..." : `Start ${selected.trialDays} day trial` : "Trial unavailable"}</button><button disabled={!selectedMethod || busy === "create_manual_payment"} onClick={() => requestJson("create_manual_payment", { planVersionId: selected.id, paymentMethodId: selectedMethod })}>{busy === "create_manual_payment" ? "Creating payment..." : "Create payment request"}</button></>}</section><section><span>PAYMENT METHOD</span><div className="subscription-methods">{methods.length ? methods.map((method: any) => <button className={selectedMethod === method.id ? "selected" : ""} key={method.id} onClick={() => setSelectedMethod(method.id)}><strong>{method.name}</strong><small>{method.type.replaceAll("_", " ")} · {method.currency}</small>{method.instructions && <p>{method.instructions}</p>}</button>) : <p>No enabled payment method is available for this plan yet.</p>}</div></section></div>}
    </section>
