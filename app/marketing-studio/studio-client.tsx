...
const designOptions = [
  ...
  { key: "neighbourhood-guide", name: "Neighbourhood Guide", category: "Lifestyle", palette: "guide", image: realPhoto("photo-1600047509782-20d39509f26d") },
  { key: "mandate-announcement", name: "Mandate Announcement", category: "Exclusive", palette: "premium", image: realPhoto("photo-1600607687644-c7171b42498b") },
  { key: "holiday-stay", name: "Holiday Stay", category: "Short let", palette: "resort", image: realPhoto("photo-1600607688066-890987f18a86") },
];
const rail = [
  { name: "Templates", icon: "▯" },
  { name: "Elements", icon: "⌘" },
...
function createDoc(property: Row | undefined, agency: Row | undefined, templateId = "signature-sale", width = 1200, height = 628): DesignDocument {
  ...
  const variantLayouts: Row = {
    ...
    "neighbourhood-guide": fromSignature({ bg: { fill: "#f8fafc" }, photo: { x: 0, y: 0, width, height: height * .5, opacity: .9 }, wash: { y: height * .5, height: height * .5, opacity: 0 }, brand: { y: height * .58, color: primary }, badge: { text: "AREA GUIDE", y: height * .66, color: accent }, title: { text: `${p.suburb || "Neighbourhood"} guide`, binding: undefined, y: height * .73, color: primary, fontSize: 48 }, details: { x: width * .58, y: height * .64, color: "#52655f", text: "Schools · shopping · lifestyle · market trends", binding: undefined }, price: { x: width * .58, y: height * .77, color: primary, text: "Explore local insight", binding: undefined }, cta: { x: width * .58, y: height * .88, color: accent } }),
    "mandate-announcement": fromSignature({ bg: { fill: "#090f0d" }, photo: { x: width * .08, y: height * .12, width: width * .42, height: height * .76, radius: 999, opacity: .92 }, wash: { opacity: .1 }, brand: { x: width * .58 }, badge: { text: "EXCLUSIVE MANDATE", x: width * .58 }, title: { x: width * .58, width: width * .32, fontSize: 50 }, details: { x: width * .58, width: width * .32 }, price: { x: width * .58, color: accent }, cta: { x: width * .58, text: "Private viewings only" } }),
    "holiday-stay": fromSignature({ bg: { fill: "#ecfeff" }, photo: { x: width * .46, width: width * .54, opacity: .95 }, wash: { fill: "#164e63", opacity: .2 }, brand: { color: "#0e7490" }, badge: { text: "SHORT LET", color: accent }, title: { color: "#ffffff", fontSize: 54 }, details: { text: "Flexible stays · furnished · secure", binding: undefined }, price: { text: "Enquire for rates", binding: undefined }, cta: { text: "Check availability" } }),
  };
...
export default function MarketingStudioClient({ platform }: { platform: { shortName: string; primaryColor: string; accentColor: string } }) {
  ...
  const [exportKind, setExportKind] = useState<"png" | "jpg" | "svg">("png"), [editingText, setEditingText] = useState(""), drag = useRef<DragState | null>(null);
  ...
  const canvasPoint = (event: { clientX: number; clientY: number }) => {
    ...
    return rect ? { x: (event.clientX - rect.left) / zoom, y: (event.clientY - rect.top) / zoom } : { x: (doc?.width || 1200) * .36, y: (doc?.height || 628) * .36 };
  };
  ...
  return <main className="studio-page" style={themeVars}>
    ...
    <section className="studio-editor-shell">
      ...
      <aside className="studio-left-panel"><span>DESIGN TOOLS</span><h2>{tool}</h2>
      ...
      <section className="studio-workspace">
        <div className="studio-workbar">
          <span>{selected ? `${selected.name} · Layer ${selectedLayerIndex}` : "LIVE PREVIEW"}</span>
          ...
          <div className="studio-zoom-controls"><button aria-label="Zoom out" onClick={() => setZoom(Math.max(.25, zoom - .1))}>−</button><select aria-label="Zoom" value={String(Math.round(zoom * 100))} onChange={(e) => setZoom(Number(e.target.value) / 100)}>{[25, 50, 75, 100, 125, 150, 200].map((v) => <option key={v} value={v}>{v}%</option>)}</select><button aria-label="Zoom in" onClick={() => setZoom(Math.min(2, zoom + .1))}>+</button><button onClick={() => setSelectedIds([])}>Preview</button></div>
        </div>
        ...
        <div className="studio-stage" onDragOver={(e) => e.preventDefault()} onDrop={dropOnCanvas} onPointerMove={move} onPointerUp={() => { drag.current = null; }} onPointerCancel={() => { drag.current = null; }}>
          <div className="studio-artboard" style={{ width: doc.width * zoom, height: doc.height * zoom }} onPointerDown={() => setSelectedIds([])}>
            <div ref={canvasRef} className="studio-canvas studio-layout-document" style={{ width: doc.width, height: doc.height, transform: `scale(${zoom})` }}>
        ...
        <div className="studio-mobile-toolbar"><button onClick={() => setTool("Templates")}>Templates</button><button onClick={() => setTool("Uploads")}>Uploads</button><button onClick={() => setTool("Text")}>Text</button><button onClick={duplicate} disabled={!selected}>Duplicate</button><button onClick={remove} disabled={!selected}>Delete</button></div>
      </section>
    </section>
