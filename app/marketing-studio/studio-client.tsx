...
export default function MarketingStudioClient({ platform }: { platform: { shortName: string; primaryColor: string; accentColor: string } }) {
  ...
  return <main className="studio-page" style={themeVars}>
    ...
    <section className="studio-editor-shell">
      ...
      <aside className={`studio-left-panel ${mobileToolsOpen ? "open" : ""}`}><span>DESIGN TOOLS</span><h2>{tool}</h2><button className="studio-mobile-tools-close" onClick={() => setMobileToolsOpen(false)} aria-label="Close toolbar">×</button><div className="studio-mobile-tool-tabs">{rail.map((item) => <button className={tool === item.name ? "active" : ""} onClick={() => { setTool(item.name); setMobileToolsOpen(true); }} key={item.name}>{item.name}</button>)}</div>
      ...
      <section className="studio-workspace">
        <div className="studio-workbar">
          <button className="studio-mobile-tools-toggle" onClick={() => setMobileToolsOpen((open) => !open)}>{mobileToolsOpen ? "Close toolbar" : "Toolbar"}</button><span>{selected ? `${selected.name} · Layer ${selectedLayerIndex}` : "LIVE PREVIEW"}</span>
