...
export default function MarketingStudioClient({ platform }: { platform: { shortName: string; primaryColor: string; accentColor: string } }) {
  ...
  const [exportKind, setExportKind] = useState<"png" | "jpg" | "svg">("png"), [editingText, setEditingText] = useState(""), [mobileToolsOpen, setMobileToolsOpen] = useState(false), [mobileExportOpen, setMobileExportOpen] = useState(false), [mobileFitZoom, setMobileFitZoom] = useState(.52), drag = useRef<DragState | null>(null);
  ...
  const isMobileCanvas = typeof window !== "undefined" && window.matchMedia("(max-width: 820px)").matches;
  const effectiveZoom = isMobileCanvas && zoom === .52 ? mobileFitZoom : zoom;
  ...
  const deleteUpload = (upload: Row) => {
    setStudioUploads((items) => {
      const next = items.filter((item) => item.id !== upload.id && item.url !== upload.url);
      try { localStorage.setItem(uploadKey, JSON.stringify(next)); } catch { setError("Upload was removed, but the browser library could not be saved."); }
      return next;
    });
  ...
  const startPinch = (event: TouchEvent) => { if (event.touches.length === 2) pinch.current = { distance: touchDistance(event), zoom: effectiveZoom }; };
  const movePinch = (event: TouchEvent) => {
    if (!pinch.current || event.touches.length !== 2) return;
    event.preventDefault();
    const next = pinch.current.zoom * (touchDistance(event) / Math.max(1, pinch.current.distance));
    setZoom(Math.max(.18, Math.min(2, Number(next.toFixed(3)))));
  };
  ...
  return <main className="studio-page" style={themeVars}>
    <header className="studio-topbar">
      ...
      <div className={`studio-mobile-export-menu ${mobileExportOpen ? "open" : ""}`}><button className="studio-mobile-export-trigger" type="button" aria-label="Export and download" onClick={() => setMobileExportOpen((open) => !open)}><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M12 3v12" /><path d="m7 10 5 5 5-5" /><path d="M5 19h14" /></svg></button>{mobileExportOpen && <div className="studio-mobile-export-panel"><button onClick={() => { setMobileExportOpen(false); void downloadPreview(); }}><strong>Download to device</strong><span>{exportKind.toUpperCase()} file</span></button><button onClick={() => { setMobileExportOpen(false); void saveExport(); }} disabled={busy}><strong>Save to platform</strong><span>Rendered files</span></button><button onClick={() => { setMobileExportOpen(false); void sharePreview(); }}><strong>Share design</strong><span>Send or copy link</span></button></div>}</div>
    </header>
    ...
    <section className="studio-editor-shell">
      ...
      <section className={`studio-workspace ${selected ? "has-mobile-selection" : ""}`}>
        <div className="studio-workbar">
          ...
          <div className="studio-zoom-controls"><button aria-label="Zoom out" onClick={() => setZoom(Math.max(.18, effectiveZoom - .1))}>−</button><select aria-label="Zoom" value={String(Math.round(effectiveZoom * 100))} onChange={(e) => setZoom(Number(e.target.value) / 100)}>{[25, 50, 75, 100, 125, 150, 200].map((v) => <option key={v} value={v}>{v}%</option>)}</select><button aria-label="Zoom in" onClick={() => setZoom(Math.min(2, effectiveZoom + .1))}>+</button><button onClick={() => { setZoom(mobileFitZoom); setSelectedIds([]); }}>Fit</button></div>
          {selected && <div className="studio-mobile-selection-actions"><button onClick={() => { setTool(selectedIsText ? "Text" : selectedIsImage ? "Uploads" : "Brand"); setMobileToolsOpen(true); }}>Edit</button><button onClick={duplicate}>Duplicate</button><button className="danger" onClick={remove}>Delete</button></div>}
        </div>
        <input ref={imageInputRef} className="studio-hidden-file" aria-label="Change image" type="file" accept="image/*" onChange={changeImage} />
        <div className="studio-stage" onDragOver={(e) => e.preventDefault()} onDrop={dropOnCanvas} onTouchStart={startPinch} onTouchMove={movePinch} onTouchEnd={endPinch} onTouchCancel={endPinch} onPointerMove={move} onPointerUp={() => { drag.current = null; }} onPointerCancel={() => { drag.current = null; }}>
      ...
      <div className={`studio-mobile-toolbar ${mobileToolsOpen ? "open" : ""}`}>{rail.map((item) => <button className={tool === item.name ? "active" : ""} onClick={() => { setTool(item.name); setMobileToolsOpen(true); }} key={item.name}><b>{item.icon}</b><span>{item.name}</span></button>)}</div>
    </section>
