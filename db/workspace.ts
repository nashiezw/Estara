...
export async function requireWorkspace(user:ChatGPTUser):Promise<WorkspaceContext>{
 ...
 if(existing){await ensureSettings(existing.agencyId);return{...existing,userId:user.userId}};
 ...
 await ensureSettings(agencyId);await seedAgency(agencyId,user.userId);return{agencyId,agencyName:"Prime Property",userId:user.userId};
}
